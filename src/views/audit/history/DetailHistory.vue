      ...
      <tr>
        ...
        <td style="width: 20%; vertical-align: top">
          <el-col style="padding-left: 5px; float: left">
            ...
            >
            <div class="title">部门相关信息</div>
              <div style="padding: 0 20px; font-weight: 300">
                <h3>分管部门校领导：{{ queryData.leaderUser.leaderUser.userName }}</h3>
                <div>
                  工号：{{ queryData.leaderUser.leaderUser.userId }}<br>
                  所属部门：{{ queryData.leaderUser.leaderUser.yuanXi }}
                </div>
              </div>
            </el-card>
...
export default {
  ...
  created() {
    this.queryData = this.$route.query; //获取跳转页面传递的参数信息
    console.log(this.queryData);
    this.department = this.queryData.leaderUser.department;  //将部门信息作为全局参数参数
    var now = new Date();
