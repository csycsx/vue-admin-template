<template>
  ...
  <div class="app-container">
    <el-card>
      ...
      <nav class="right">
        <el-button type="primary" @click="addVocationVisible = true">+添加</el-button>
      </nav>
      <div class="block">
        <el-date-picker v-model="text" type="year" placeholder="选择年份信息进行查阅" format="yyyy">
        </el-date-picker>
      ...
      <!-- 表单 -->
      <el-table border :data="calenderList" style="width: 100%">
        ...
        <el-table-column label="假期名称" prop="holidayName">
        </el-table-column>
        <el-table-column type="date" prop="holidayStartDate" label="开始日期" :formatter="dateFormat">
        </el-table-column>
        <el-table-column type="date" prop="holidayEndDate" label="结束日期" :formatter="dateFormat">
        </el-table-column>
      ...
      <!-- 新增信息 -->
      <el-dialog title="新增假期" :visible.sync="addVocationVisible" width=30%>
        ...
        <el-input v-model="addForm.userName"  disabled=true style="width:200px"></el-input>
          ...
          <el-form-item label="描述信息" prop='description' :label-width="formLabelWidth">
            <el-select style="width:200px" v-model="addForm.description" placeholder="" class="select-update1">
          ...
          <el-form-item label="假期名称" prop='holidayName' :label-width="formLabelWidth">
            <el-select style="width:200px" allow-create filterable v-model="addForm.holidayName" placeholder=""
              class="select-update2" @change="areaChange1">
              <span v-if="addForm.description">
                <span v-for="item in regionList" :key="item.value">
                  <span v-if="item.value === addForm.description">
                    <el-option v-for="item1 in item.child" :key="item1.value" :label="item1.label" :value="item1.value">
                    </el-option>
                  </span>
                </span>
              </span>
