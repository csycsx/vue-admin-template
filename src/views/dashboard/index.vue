...
export default {
  ...
  created () {
    /*
      data 对象是存储在前端的数据，一般请求的时候需要这个信息做验证查验，
      example调用的接口是get，所以没有传回后端
    */
    var data = {
      id: this.$store.getters.id, // 获取 store中的 id
      name: this.$store.getters.name, // 获取 store中的 name
